        ...
        <div class="comment" data-id="${c.id}">
            ...
            <div class="comment__body">
                ...
                <div class="comment__actions">
                    ...
                    <button class="comment__action-btn report-btn" data-id="${c.id}">Report</button>
...
function bindCommentEvents(threadEl, currentUser, storyId, chapterId, reload) {
    ...
    // Report buttons
    threadEl.querySelectorAll('.report-btn').forEach(btn => {
        btn.addEventListener('click', async function () {
            if (!confirm('Report this comment?')) return;
            const commentId = this.dataset.id;
            await waitForSupabase();
            const { data } = await window.supabase
                .from('comments').select('report_count').eq('id', commentId).single();
            await window.supabase.from('comments')
                .update({ report_count: (data?.report_count || 0) + 1 })
                .eq('id', commentId);
            this.textContent = 'Reported';
            this.disabled = true;
            this.style.color = 'var(--color-warning)';
        });
    });
